...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    /**
     * LOOKING   正在进行选举过程
     * FOLLOWING  表明当前角色是follower
     * LEADING   表明当前角色是leader
     * OBSERVING 表明当前角色是observer
     */
    ...
    public synchronized void start() {
        //加载数据
        loadDataBase();
        //开启读取数据线程  默认使用NIOServerCnxnFactory组委实现类  里面使用nio处理socket请求
        cnxnFactory.start();
        //进行领导者选举，确定服务器的角色，在针对不同的服务器角色进行初始化
        startLeaderElection();   //初始化领导者选举
        //启动线程
        super.start();
    }
    ...
    protected Election createElectionAlgorithm(int electionAlgorithm){
        ...
        case 3:
            ...
            if(listener != null){
                listener.start();
                //默认使用的是FastLeaderElection 作为实现类
